...
def create_conv_model(batch, in_c, out_c, h, w, kernel_size=3):
    ...
    model = helper.make_model(graph, opset_imports=[helper.make_opsetid('', 13)])
    return model, {'X': (batch, in_c, h, w), 'W': (out_c, in_c, kernel_size, kernel_size)}
...
def run_and_capture_shader(model_path, input_shapes):
    """Run inference and capture shader output."""
    # Build inputs string
    inputs_code = ""
    for name, shape in input_shapes.items():
        inputs_code += f'inputs["{name}"] = np.random.randn{shape}.astype(np.float32)\n'
...
def main():
    ...
    if args.op == 'matmul':
        if not args.quiet:
            print(f"Operator: MatMul ({args.m} x {args.k}) @ ({args.k} x {args.n})")
        model, input_shapes = create_matmul_model(args.m, args.k, args.n)
    elif args.op == 'add':
        if not args.quiet:
            print(f"Operator: Add (shape: {args.shape})")
        model, input_shapes = create_add_model(args.shape)
    elif args.op == 'softmax':
        if not args.quiet:
            print(f"Operator: Softmax (shape: {args.shape})")
        model, input_shapes = create_softmax_model(args.shape)
    elif args.op == 'conv':
        if not args.quiet:
            print("Operator: Conv2D (1, 64, 64, 224, 224)")
        model, input_shapes = create_conv_model(1, 64, 64, 224, 224)
    ...
    try:
        ...
        output = run_and_capture_shader(model_path, input_shapes)
